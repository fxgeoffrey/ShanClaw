...
### Context Management
- **Context window**: seeded from `agent.context_window` (default 200000) via `loop.SetContextWindow`, then **runtime-auto-adjusted from observed `response.model`** via `loop.maybeAutoAdjustContextWindow` after every main-tier LLM response. The seed is correct for turn 1; from turn 2 onward the threshold reflects the actual model serving the request (Anthropic / OpenAI / Google / xAI windows in `internal/agent/modelcontext.go`). Cloud-side tier-failover scenarios (medium priority-1 sonnet-4-6 1M → fallback sonnet-4-5 200K) are caught automatically: the loop shrinks contextWindow on the response that first surfaces the smaller model, so the next prompt's preflight check uses the right threshold.
  - **Lookup table** is symmetric on 1M / 200K families and supports both dateless floating tags (e.g. `claude-sonnet-4-6`, `claude-sonnet-4-5`) and dated variants via longest-prefix-first matching (`prefixLookupOrder`). Unknown model IDs return `(0, false)` and leave the existing window untouched — graceful degradation rather than silent down-cap.
  - **Per-agent override** (`agent.model` + `agent.context_window` in `~/.shannon/agents/<name>/config.yaml`) calls `SetContextWindowExplicit`, which raises a lock flag; auto-detect skips locked loops so cost-cap pins stick. The plain `SetContextWindow` (global / seed path) intentionally clears the lock so a soft reseed flows back to auto mode.
  - **No `anthropic-beta` header is sent** (retired 2026-04-30; sonnet-4-6 / opus-4-6 / 4-7 are GA at 1M, standard pricing). Ollama / custom-cap operators should set `agent.context_window` on a per-agent override so auto-detect does not override a local-model cap to the wrong Claude window.
