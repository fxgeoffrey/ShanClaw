...
- **Empty-think force-stop** (`internal/agent/loopdetect.go` rule "0a" at top of `Check()`): two consecutive `think` calls whose parsed args yield an empty/whitespace `thought` → immediate `LoopForceStop`. Detection lives on `ToolCallRecord.IsEmptyThinkInput`, set by `Record()` via the `isEmptyThinkInput` helper. Paired with `internal/tools/think.go` soft-handling empty thought (`IsError: false`, hint content) so the rule fires on the same conditions the tool itself treats as no-op. Defends against Sonnet 4.6 / Opus 4.7 emitting ritual `think({})` calls after native interleaved thinking blocks (one production hang showed 4 consecutive empties in 14 minutes). See `docs/superpowers/plans/2026-05-14-thinking-blocks-cc-alignment.md` Phase 0 for the wider plan this bottom-guards.
- **Thinking blocks transport** (cross-repo wire change, 2026-05): Shannon Cloud's `anthropic_provider.py` now relays the assistant message's full ordered `content_blocks: [...]` list — including `thinking` / `redacted_thinking` — instead of dropping them. ShanClaw's `client.ContentBlock` carries `Thinking` / `Signature` / `Data` fields; `client.CompletionResponse.ContentBlocks []ContentBlock` receives the wire list. `agent.buildAssistantMessage(resp, normalizedToolText)` prefers `resp.ContentBlocks` (verbatim copy with order + signatures) and falls back to `OutputText` + `AllToolCalls()` for older Cloud. Sanitizers in `messagesForLLM` / time-based / micro-compaction / `BuildForkedRequest` all keep thinking blocks intact (invariant tests in `loop_test.go` / `forkedrequest_test.go`). Session JSON persists them verbatim; `internal/sync/strip_thinking.go` removes them from the upload-side copy BEFORE the size-limit check (privacy + size-cap fairness). Satisfies Anthropic's "thinking blocks must be preserved unmodified across the assistant trajectory" + handles interleaved thinking emitted between consecutive `tool_use` blocks.
- **Conditional `think` tool** (`internal/tools/register.go` `shouldRegisterThinkTool`): the local `think` tool is no longer registered on the default gateway+thinking path — native extended thinking covers the same role and Anthropic's own Claude Code ships zero think-equivalent tools. Still registered when `cfg.Agent.Thinking == false` (no native fallback), `cfg.Provider == "ollama"` (OpenAI-shape API has no thinking blocks), or `cfg.Agent.ForceThinkTool == true` (explicit user override). Prompt-side: `agent/loop.go:operationalRules()` strips the `### Planning` bullet from the system prompt when think is not in the live registry (via `ToolRegistry.Has`), keeping the prompt byte-equal to the previous `coreOperationalRules` const whenever think IS registered (no prompt-cache prefix drift).
